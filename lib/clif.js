...
//
clif.original_key = clif.key;

//
// which algorithm to use to encrypt the key
//
clif.cipher_type  = 'aes-256-cbc';
...
clif.pp = function (level, msg, opts) {
  ...
  if(_k) {
    ...
    if(clif.argv.raw) {
      ...
      return _n + msg[clif.msg[level].color];
    }
  }
};
...
clif.aborted = function (msg) {
  clif.err('aborted by you. ' + '['.blue + msg + ']'.blue);
};
...
//
clif.err = function(err, newline) {
  clif.logger(
    clif.pp('err', (err.message || err), 
    { newline: newline
    , key: 'error'
    })
  );
...
clif.encrypt = function (wat) {
  if(VERBOSE) {
    if (clif.key === clif.original_key) {
      clif.silly('you should use a different key', 'key');
    }
    clif.silly('encrypting ' + wat.length + ' chars');
  }

  try {
    var crypto  = require('crypto')
      , cipher  = crypto.createCipher(clif.cipher_type, clif.key)
    ...
    if(VERBOSE) {
      clif.silly(wat + ' failed to encrypt: ' + e.message);
    }
    return wat;
  }
};
...
clif.decrypt = function (encoded) {
  if(VERBOSE) {
    ...
    clif.silly('decrypting ' + encoded);
  }

  try {
    var crypto   = require('crypto')
      , decipher = crypto.createDecipher(clif.cipher_type, clif.key)
    ...
    if(VERBOSE) {
      clif.silly(encoded + ' failed to decrypt: ' + e.message);
    }
    return encoded;
  }
};
...
//
var key_path = path.join(clif.tilde, '.ssh', 'id_rsa');

try {
  key = fs.readFileSync(key_path, 'utf-8');
  if(VERBOSE) {
    clif.silly('loaded rsa key from ' + key_path);
  }
} catch (e) {
  if(VERBOSE) {
    clif.silly('user private key was not in ' + key_path + 
      '. using default key');
  }
}
...
clif.load_dotfile = function () {
  ...
  try {
    ...
    Object.keys(cfg).forEach(function (k) {
      for(var i=0; i < clif.sensitive_options.length; i++) {
        var sensitive_word = clif.sensitive_options[i].toLowerCase();
...
clif.save_dotfile = function (updates, cfg) {
  if(!SAVE) {
    return;
  }
  ...
  if(!Object.keys(updates).length) {
    if(VERBOSE) {
      clif.silly('nothing to do cause there are no updates');
    }
    return;
  }
...
clif.main = function() {
  ...
  var prompt_info = [].reduce.call(arguments, function(ac, arg) {
    ...
    //
    clif.reserved_options.forEach(function (word) {
      if (prop.name === word) {
        clif.err(
    ...
    //
    clif.sensitive_options.forEach(function(word) {
...
clif.relax = clif.main;
clif.dinosaur = clif.main;
...
clif.encrypt = function (wat) {
  if(VERBOSE) {
    if (clif.key === clif.original_key) {
      clif.warn('you should use a different key', 'key');
    }
    clif.info('encrypting ' + wat.length + ' chars');
  }
  ...
  try {
    ...
    if(VERBOSE) {
      clif.err(wat + ' failed to encrypt: ' + e.message);
    }
    return wat;
  }
};
...
clif.decrypt = function (encoded) {
  ...
  try {
    ...
    if(VERBOSE) {
      clif.err(encoded + ' failed to decrypt: ' + e.message);
    }
    return encoded;
  }
};

...
frameless = function relax() {
  ...
  frameless.load_dotfile(function (cfg) {
    ...
    //
    var prompt_info = [].reduce.call(args, setup_params, []);

    function setup_params(ac, arg) {
      //
      ...
      //
      if(util.isArray(prop.properties)) {
        prop.properties.forEach(function(p) {
          p.name = prop.name + '-' + p.name;
          setup_params(ac, p);
        });
        return ac;
      }
    ...
    //
    // Parses nested options
    //
    function parse_nested(opts) {
      ...
      Object.keys(opts).forEach(function(opt) {
        if(~opt.indexOf('-')) {
          ...
          var subkey = params.join('-');

          parsed_opts[key] = parsed_opts[key] || {};
          parsed_opts[key][subkey] = value;

          delete parsed_opts[opt];
        }
      });
      return parsed_opts;
    }
